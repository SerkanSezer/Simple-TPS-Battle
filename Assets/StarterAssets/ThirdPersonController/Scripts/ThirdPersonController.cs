...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            audioSource = playerAudio.audioSourceFootStep;
        ...
        private void Start()
        {
            ...
            _input.SetCursorState(cursorFlag);
        }
        ...
        private void SetSelectMenu() {
            if (_input.select) {
                ...
                _input.SetCursorState(cursorFlag);
                if (!cursorFlag) { 
                    inventoryUI.GetComponent<Image>().color = new Color(0, 0, 0, 0.7f);
                }
                else {
                    inventoryUI.GetComponent<Image>().color = new Color(0, 0, 0, 0.2f);
                }
                _input.select = false;
            }
        }
        ...
        private void Move()
        {
            ...
            if (_animationBlend > 0) {
                if (_animationBlend > 1f && _animationBlend < 2.1f) {
                    audioSource.pitch = 0.8f;
                    audioSource.clip = playerAudio.GetWalkSound();
                    if(!audioSource.isPlaying) audioSource.Play();
                }else if(_animationBlend > 2.1f) {
                    audioSource.pitch = 1f;
                    audioSource.clip = playerAudio.GetRunSound();
                    if (!audioSource.isPlaying) audioSource.Play();
                }
            }
            else {
